...
public class orbitBug : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        transform.position = new Vector3(transform.position.x, min, transform.position.z);  //keep bug above ground
        transform.rotation = Quaternion.LookRotation(player.transform.position) * rotationOffset;            //bug looks at player
