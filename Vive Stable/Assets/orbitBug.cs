...
public class orbitBug : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        transform.RotateAround(GameObject.Find("FallbackObjects").transform.position, new Vector3(0, 3, 0), rotationSpeed * Time.deltaTime);
        var desiredPosition = (transform.position - GameObject.Find("FallbackObjects").transform.position).normalized * radius + GameObject.Find("FallbackObjects").transform.position;
    ...
    void OnTriggerEnter(Collider collision)
    {
        ...
        if (collision.gameObject.name == "FallbackObjects")
        {
            gameObject.SetActive(false);
